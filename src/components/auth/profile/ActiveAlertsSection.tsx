...
export function ActiveAlertsSection({ 
  ...
  const { data: currentPrices } = useQuery({
    queryKey: ["currentPrices", alerts.map(a => a.id)],
    queryFn: async () => {
  ...
  const sortedAlerts = [...alerts].sort((a, b) => {
    if (renewablePreference) {
      // Sort by renewable percentage first, then by price threshold
  ...
  return (
    <div className="max-h-[calc(100vh-24rem)] overflow-y-auto rounded-md border p-4">
      <div className="space-y-4">
        {sortedAlerts.map((alert) => {
          const currentPrice = currentPrices?.[alert.plan_id]?.[`price_kwh${alert.kwh_usage}`];
